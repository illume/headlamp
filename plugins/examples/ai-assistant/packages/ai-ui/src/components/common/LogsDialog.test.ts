...
import { getLogLanguage, getFormattedLogs, buildLogTitle } from '../../formatting/logFormatting';
...
describe('buildLogTitle', () => {
  ...
  it('includes namespace when no container name', () => {
    expect(buildLogTitle('Pod', 'my-pod', undefined, 'default')).toBe(
      'Pod my-pod (default) Logs'
    );
  });
