...
describe('ToolConfigManager', () => {
  ...
  describe('isToolEnabled', () => {
    ...
    it('returns true when a tool is explicitly enabled', () => {
      expect(isToolEnabled({ enabledTools: { [builtInTool.id]: true } }, builtInTool.id)).toBe(true);
    });
  ...
  it('getEnabledToolIds returns enabled built-in tool IDs', () => {
    ...
    expect(getEnabledToolIds({ enabledTools: { [builtInTool.id]: true } })).toEqual([builtInTool.id]);
  });
