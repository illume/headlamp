...
export class MCPEmbeddingRouter {
  ...
  async indexTools(tools: MCPToolInfo[]): Promise<void> {
    ...
    try {
      ...
      console.warn('MCPEmbeddingRouter: failed to index tools, will fall back to keyword routing:', error);
      this.embeddedTools = [];
  ...
  async route(
    ...
    try {
      ...
      console.warn('MCPEmbeddingRouter: query embedding failed, falling back to keyword routing:', error);
      return routeMCPTools(query, tools, config);
    }
  }
