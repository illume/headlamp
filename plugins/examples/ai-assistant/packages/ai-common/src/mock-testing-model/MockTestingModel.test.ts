...
describe('parseTemplate', () => {
  ...
  it('handles multiple variables', () => {
    const { parts, varNames } = parseTemplate(
      'Delete <<resource>> <<name>> in <<ns>>'
    );
...
describe('matchTemplate', () => {
  ...
  it('captures multiple variables', () => {
    expect(
      matchTemplate(
        'Delete service my-svc in production',
        'Delete <<resource>> <<name>> in <<ns>>'
      )
  ...
  it('is case-insensitive for literal parts', () => {
    expect(
      matchTemplate('WHAT IS A Deployment?', 'What is a <<resource>>?')
    ).toEqual({ resource: 'Deployment' });
  });

  it('returns undefined when a literal part is missing', () => {
    expect(
      matchTemplate('Tell me about a Pod', 'What is a <<resource>>?')
    ).toBeUndefined();
  });
...
describe('fillTemplate', () => {
  it('substitutes a single variable', () => {
    expect(fillTemplate('A <<resource>> is cool', { resource: 'Pod' })).toBe(
      'A Pod is cool'
    );
  });
  ...
  it('substitutes multiple occurrences of the same variable', () => {
    expect(
      fillTemplate('<<x>> and <<x>> again', { x: 'hello' })
    ).toBe('hello and hello again');
  });

  it('substitutes multiple different variables', () => {
    expect(
      fillTemplate('<<a>> meets <<b>>', { a: 'Alice', b: 'Bob' })
    ).toBe('Alice meets Bob');
  });

  it('leaves unknown variables unchanged', () => {
    expect(fillTemplate('<<known>> and <<unknown>>', { known: 'yes' })).toBe(
      'yes and <<unknown>>'
    );
  });
});
...
describe('matchFixtures', () => {
  ...
  it('matches a template fixture and substitutes variables', () => {
    expect(matchFixtures('What is a Service?', fixtures)).toBe(
      'A Service is a K8s resource.'
    );
  });

  it('matches multiple variables', () => {
    expect(matchFixtures('Scale nginx to 5 replicas', fixtures)).toBe(
      'Scaling nginx to 5.'
    );
  });
...
describe('createMockTestingModel', () => {
  ...
  it('returns fallback for unmatched prompts', async () => {
    ...
    const result = await model.invoke([
      new HumanMessage('Tell me about quantum computing'),
    ]);
  ...
  it('uses extraFixtures before built-ins', async () => {
    const model = createMockTestingModel({
      extraFixtures: [
        { prompt: 'Hello', response: 'Custom hello!' },
      ],
    });
    const result = await model.invoke([new HumanMessage('Hello')]);
