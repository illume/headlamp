...
describe('contextGenerator', () => {
  ...
  describe('generateContextDescription', () => {
    ...
    it('includes cluster warnings and warning resources in structured context', () => {
      ...
      expect(result).toContain('RESOURCES IN CONTEXT (use the resource name as the markdown link text):');
    ...
    it('includes structured resources from resource, items, and resources', () => {
      ...
      expect(result).toContain('- kind: Service, name: frontend, namespace: default, cluster: primary');
      expect(result).toContain('- kind: Deployment, name: web, namespace: default, cluster: remote');
    });
  });
