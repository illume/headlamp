...
import { routeSkills, SkillRouterConfig, DEFAULT_ROUTER_CONFIG, ScoredSkill } from './SkillRouter';
...
export class EmbeddingRouter {
  ...
  async indexSkills(skills: ParsedSkill[]): Promise<void> {
    ...
    try {
      ...
      console.warn('EmbeddingRouter: failed to index skills, will fall back to keyword routing:', error);
      this.embeddedSkills = [];
  ...
  async route(
    ...
    try {
      ...
      console.warn('EmbeddingRouter: query embedding failed, falling back to keyword routing:', error);
      return routeSkills(query, skills, config);
    }
  }
