...
describe('SkillRouter', () => {
  ...
  describe('routeSkills', () => {
    const skills: ParsedSkill[] = [
      makeSkill('install', 'Kubeshark installation and deployment skill', 'Helm install content', ['kubeshark', 'helm', 'install']),
      makeSkill('network-rca', 'Kubernetes network root cause analysis with Kubeshark', 'Snapshot and KFL content', ['kubernetes', 'network', 'rca']),
      makeSkill('helmfile', 'Expert guidance for Helmfile declarative Helm chart deployment', 'Helmfile content', ['helm', 'helmfile', 'deployment']),
      makeSkill('node-not-ready', 'Troubleshoot NotReady or SchedulingDisabled node status', 'Node troubleshooting', ['kubernetes', 'node', 'troubleshooting']),
      makeSkill('pod-failure-diagnosis', 'Troubleshoot CrashLoopBackOff, ImagePullBackOff, Pending, Error, or OOMKilled', 'Pod troubleshooting', ['kubernetes', 'pod', 'crashloop']),
      makeSkill('kubernetes-security', 'Kubernetes RBAC and security', 'K8s security', ['kubernetes', 'security']),
    ];
  ...
  describe('scoreSkills', () => {
    const skills: ParsedSkill[] = [
      makeSkill('install', 'Kubeshark installation and deployment', 'Install content', ['kubeshark']),
      makeSkill('helmfile', 'Helmfile declarative Helm deployment', 'Helmfile content', ['helm']),
      makeSkill('node-not-ready', 'Kubernetes node troubleshooting', 'Node content', ['kubernetes']),
    ];
  ...
  describe('routeAndFormatSkills', () => {
    const skills: ParsedSkill[] = [
      makeSkill('install', 'Kubeshark installation and deployment', 'Install workflow instructions'),
      ...
      makeSkill('node-not-ready', 'Kubernetes node troubleshooting NotReady', 'Node troubleshooting content'),
      makeSkill('pod-failure-diagnosis', 'Pod CrashLoopBackOff troubleshooting', 'Pod troubleshooting content'),
