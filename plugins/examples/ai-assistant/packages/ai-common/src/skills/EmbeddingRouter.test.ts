...
describe('EmbeddingRouter', () => {
  const skills: ParsedSkill[] = [
    makeSkill('install', 'Kubeshark installation and deployment', 'Helm install content', ['kubeshark', 'helm', 'install']),
    makeSkill('network-rca', 'Kubernetes network root cause analysis', 'Snapshot and KFL content', ['kubernetes', 'network', 'rca']),
    makeSkill('helmfile', 'Helmfile declarative Helm chart deployment', 'Helmfile content', ['helm', 'helmfile', 'deployment']),
    makeSkill('node-not-ready', 'Troubleshoot NotReady node status', 'Node troubleshooting', ['kubernetes', 'node', 'troubleshooting']),
    makeSkill('pod-failure', 'Troubleshoot CrashLoopBackOff OOMKilled', 'Pod troubleshooting', ['kubernetes', 'pod', 'crashloop']),
    makeSkill('security', 'Kubernetes RBAC security analysis', 'K8s security', ['kubernetes', 'security', 'rbac']),
  ];
