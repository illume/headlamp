...
Content`,
  ...
  describe('loadFromGitRepo', () => {
    it('should throw if http client not configured', async () => {
      ...
      const loader = new SkillLoader(fs);
      await expect(
        loader.loadFromGitRepo('https://github.com/owner/repo')
      ).rejects.toThrow('HTTP client and ZIP extractor are required');
    });
    ...
    it('should reject invalid URLs', async () => {
      ...
      await expect(
        loader.loadFromGitRepo('http://evil.com/repo')
      ).rejects.toThrow('Invalid or disallowed Git URL');
    });
