...
/**
 * AKS agent terminal output parsing and normalization.
 *
 * All functions in this module are pure business logic with no
 * headlamp-plugin dependency. They transform raw terminal output
 * from the AKS agent into clean markdown suitable for display.
 */
 ...
 */
export function blockContainsCode(blockLines: string[]): boolean {
 ...
 */
export function isProseHeadingEndingWithColon(trimmed: string): boolean {
  return (
    /:\s*$/.test(trimmed) &&
    !/^[\w][\w.\/-]*:/.test(trimmed) &&
 ...
 */
export function hasShellSyntax(trimmed: string): boolean {
 ...
 */
export function isFileHeaderComment(trimmed: string): boolean {
 ...
 */
export function isBoldFileHeading(trimmed: string): boolean {
 ...
 */
export function looksLikeShellOrDockerCodeLine(trimmed: string): boolean {
 ...
 */
export function collapseTerminalBlankLines(text: string): string {
 ...
 */
export function normalizeTerminalMarkdown(text: string): string {
  const lines = text.split('\n');
  const result: string[] = [];
  let inCodeFence = false;
 ...
 */
export function looksLikeYaml(trimmed: string): boolean {
  if (
    trimmed === '' ||
    (trimmed.startsWith('#') &&
...
export function wrapBareYamlBlocks(text: string): string {
  const lines = text.split('\n');
  const result: string[] = [];
  let i = 0;
  let inCodeFence = false;
 ...
 */
export function hasStructuredCodeContext(codeLines: string[]): boolean {
 ...
 */
export function wrapBareCodeBlocks(text: string): string {
  const lines = text.split('\n');
  const result: string[] = [];
  let i = 0;
  let inCodeFence = false;
 ...
 */
export function cleanTerminalFormatting(text: string): string {
  const lines = text.split('\n');
  const result: string[] = [];
  let inCodeFence = false;
...
export function isAgentNoiseLine(trimmedLine: string): boolean {
 ...
 */
export function stripAgentNoise(lines: string[]): string[] {
 ...
 */
export function stripCommandEcho(lines: string[]): string[] {
 ...
 */
export function extractAIAnswer(rawOutput: string): string {
