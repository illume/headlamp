 ...
 */
export function verifyPluginCommandPolicies(
  ...
  return policies.filter(policy => {
    ...
    try {
      ...
      return (
        ...
        verifyPluginInstallationIntegrity(bundlePath, policy.artifactHub)
      );
    } catch {
      return false;
    }
...
export function setupRunCmdHandlers(
  ...
  ipcMain.handle('register-plugin-command-capabilities', (event, registrations: unknown) => {
    ...
    const result = createProductCommandCapabilities(
      registrations,
      verifyPluginCommandPolicies(enabledPolicies, pluginRoots),
