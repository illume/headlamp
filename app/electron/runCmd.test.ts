...
describe('verifyPluginCommandPolicies', () => {
  ...
  it('accepts an exact package identity from a regular shipped bundle', async () => {
    ...
    try {
      const result = await verifyPluginCommandPolicies(policies, {
  ...
  it('rejects missing, spoofed, or escaping shipped identities', async () => {
    ...
    try {
      ...
      expect(await verifyPluginCommandPolicies(policies, roots)).toEqual([]);
      expect(
      ...
      expect(
        await verifyPluginCommandPolicies(policies, {
          ...roots,
  ...
  it('fails closed when Electron resourcesPath is unavailable', async () => {
    ...
    try {
      expect(await verifyPluginCommandPolicies(policies)).toEqual([]);
    } finally {
  ...
  it('requires matching app-owned Artifact Hub provenance for managed plugins', async () => {
    ...
    try {
      verifyPluginInstallationIntegrityMock.mockReturnValueOnce(false);
      expect(await verifyPluginCommandPolicies([managedPolicy], roots)).toEqual([]);
      expect(verifyPluginInstallationIntegrityMock).toHaveBeenCalledWith(
        bundle,
        managedPolicy.artifactHub
      );

      verifyPluginInstallationIntegrityMock.mockReturnValueOnce(true);
      expect(await verifyPluginCommandPolicies([managedPolicy], roots)).toEqual([managedPolicy]);
    } finally {
      fs.rmSync(root, { recursive: true });
    }
  });
