...
- **Headless (no terminal, e.g., desktop icon):** Backend opens a cross-origin popup consent dialog (on a separate port). Popup is inherently immune to clickjacking and has better screen reader / keyboard accessibility. If popup blockers prevent the popup, falls back to cross-origin iframe with typed confirmation code. Cross-origin isolation (Same-Origin Policy + Origin header check) prevents plugins from accessing the popup/iframe DOM or calling the consent endpoint. See [detailed security analysis](./backend-run-command-proposal.md#approach-3-cross-origin-consent-on-a-different-port--secure-) with popup vs iframe comparison including UX, a11y, and security tradeoffs.
- **Why not a React dialog?** A React consent dialog runs in the same JavaScript context as plugins. A malicious plugin could programmatically approve it, auto-click it, or bypass it entirely by calling the API directly. Same-origin popups are equally insecure. Only cross-origin contexts (different port) provide browser-enforced isolation. See [full analysis](./backend-run-command-proposal.md#approach-1-in-page-react-dialog--not-secure-).
