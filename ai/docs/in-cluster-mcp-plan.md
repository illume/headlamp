...
- **Headless (no terminal, e.g., desktop icon):** Backend shows a cross-origin iframe consent dialog (on a separate port) with a typed confirmation code. Falls back to cross-origin popup if iframe is removed from DOM. Cross-origin isolation (Same-Origin Policy + Origin header check) prevents plugins from accessing the iframe DOM or calling the consent endpoint. The typed code defeats clickjacking. See [detailed security analysis](./backend-run-command-proposal.md#approach-3-cross-origin-consent-on-a-different-port--secure-) with iframe vs popup comparison.
