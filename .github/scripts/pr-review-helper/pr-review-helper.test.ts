...
const { COMMIT_GUIDELINES_MESSAGE, commentOnCommitGuidelineProblems, hasCommitGuidelineProblems } = require('./comment-on-commit-guideline-problems.ts');
const { MERGE_MAIN_MESSAGE, commentOnMergeMainCommit, isMergeMainCommit } = require('./comment-on-merge-main-commit.ts');
const { SNAPSHOT_MESSAGE, bufferFromResponseData, downloadJobLogs, handleWorkflowRun, hasFailedSnapshots, unzipLogArchive } = require('./comment-on-failed-frontend-snapshots.ts');
const { MARKERS, commentOnce, requestChangesOnce } = require('./github-helpers.ts');
const { dryRunGitHubClient, localPullRequestContext, parseCliArgs } = require('./pr-review-helper.ts');
...
test('skips duplicate issue comments when an idempotency marker exists', async () => {
  ...
  assert.equal(
    await commentOnce(github, 'illume', 'headlamp', 110, [{ body: MARKERS.commitGuidelines }], MARKERS.commitGuidelines, 'message'),
    false
  );
...
test('skips duplicate request-changes reviews when an idempotency marker exists', async () => {
  ...
  assert.equal(
    await requestChangesOnce(github, 'illume', 'headlamp', 110, [{ body: MARKERS.copilotComments }], MARKERS.copilotComments, 'message'),
    false
  );
  assert.equal(created, 0);
});
...
test('skips duplicate Copilot review requests when Copilot is already requested', async () => {
  ...
  assert.equal(
    await requestCopilotReview(github, 'illume', 'headlamp', 110, { requested_reviewers: [{ login: 'copilot-pull-request-reviewer' }] }, core),
    false
  );
...
test('keeps commit-guideline and merge-main helper comments idempotent', async () => {
  ...
  await commentOnCommitGuidelineProblems(github, 'illume', 'headlamp', 110, commitGuidelineData);
  assert.equal(await commentOnMergeMainCommit(github, 'illume', 'headlamp', 110, mergeMainData), false);
  assert.equal(created, 0);
});
...
test('keeps snapshot helper comments idempotent', async () => {
  ...
  await handleWorkflowRun(
    github,
    {
      ...
      payload: { workflow_run: { id: 1, conclusion: 'failure', pull_requests: [{ number: 110 }] } },
    },
    core
  );
  assert.equal(created, 0);
...
test('does not scan raw zip bytes when log extraction fails', async () => {
  let warned = false;
  const github = githubMock({ logData: Buffer.from('not a zip with snapshot failed text') });
...
test('parses local and dry-run CLI arguments', () => {
  assert.deepEqual(parseCliArgs(['--repo', 'illume/headlamp', '--pull', '110', '--dry-run']), {
...
test('dry-run mode logs writes without mutating GitHub', async () => {
  ...
  const dryRunGithub = dryRunGitHubClient(githubMock({ onCreateComment: () => created++ }), {
    info(message: string) {
  ...
  await dryRunGithub.rest.issues.createComment({ owner: 'illume', repo: 'headlamp', issue_number: 110, body: SNAPSHOT_MESSAGE });
  assert.equal(created, 0);
