...
/**
 * Total number of shard files. Each shard file tests a portion of stories.
 * This must match the number of storybook.test.shard-*.tsx files.
 */
export const STORYBOOK_SHARD_COUNT = 10;

/**
 * Runs storybook snapshot tests for a shard.
 *
 * Stories are split across multiple shard files to enable parallel execution.
 * Each shard file loads all story files but only tests its own subset,
 * determined by `shardIndex % STORYBOOK_SHARD_COUNT`.
 *
 * Vitest distributes shard files across worker threads, so multiple shards
 * run simultaneously on multi-core machines.
 */
export function runStorybookTestShard(
  storyFiles: Record<string, StoryFile>,
  shardIndex: number
) {
  const allStoryEntries = Object.entries(storyFiles).map(([filePath, storyFile]) => {
    const storyDir = path.dirname(filePath);
    const componentName = path.basename(filePath).replace(/\.(stories|story)\.[^/.]+$/, '');
    return { filePath, storyFile, componentName, storyDir };
  });

  // Filter to this shard's stories
  const shardEntries = allStoryEntries.filter(
    (_, i) => i % STORYBOOK_SHARD_COUNT === shardIndex
  );
  ...
  describe('Storybook Tests', () => {
    ...
    shardEntries.forEach(({ storyFile, componentName, storyDir }) => {
